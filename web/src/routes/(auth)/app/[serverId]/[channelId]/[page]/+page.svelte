...
<div class="bg-black opacity-90 text-center text-xl font-bold py-2 px-4 rounded-lg">
    Channels
    <div class="flex items-center text-center list text-xg justify-between flex-wrap p-3 rounded-lg space-x-5 sticky top-0">
...
{#if data.images?.length > 0 || data.videos?.length > 0}
    ...
    <button title="Shuffle" class="bg-blue-500 hover:bg-blue-600 text-white text-xl font-bold py-2 px-4 rounded-lg"
    on:click={() => {data.images = data.images.sort(() => Math.random() - 0.5); data.videos = data.videos.sort(() => Math.random() - 0.5)}}>Shuffle</button>
