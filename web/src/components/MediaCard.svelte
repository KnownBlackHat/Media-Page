<script>
	...
	function unhideControls() {
		...
		timeoutCanceler = setTimeout(() => {
			controlsVisiblity = false;
		}, 5000);
	}
		...
		{:else}
			...
			<div class="relative" id="lower-block" class:invisible={!controlsVisiblity}>
				<div class="absolute bottom-0 left-0 right-0 w-full">
					...
					<div
						class="flex gap-0.5 p-0.5 align-center items-center cursor-pointer"
						title="Player Controls"
					>
						<button
							class="text-white rounded h-5 inline-block"
						...
						>
						<div
							class="mx-2 text-white rounded inline-block"
							title="Playback Rate"
							on:click={() => {
								playbackRate === 3.0 ? (playbackRate = 1) : playbackRate++;
							}}
							role="button"
							tabindex="0"
							on:keydown={(e) => {
						...
						<button
							class="text-white rounded h-5 mx-2"
							title="Full Screen"
