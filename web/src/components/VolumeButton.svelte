...
function progressScript(volume) {
    let color;
    if (volume >= 80) {
        color = 'green';
    } else if (volume <= 30) {
  ...
  controls.style.background = `linear-gradient(to right, ${color} ${volume}%, #ccc ${volume}%)`;
  media.volume = controls.value / 100;
}

$: controls && progressScript(volumeval);
...
<div class="flex items-center justify-center">
  ...
  >
      ...
      <div class="rounded-md shadow-lg flex h-10 transform transition-all duration-500 ease-in-out origin-left"
      class:w-0={!controlsVisiblity}
      class:scale-0={!controlsVisiblity}
      on:mouseenter={() => controlsVisiblity = true}
      on:mouseleave={() => controlsVisiblity = false}
      on:blur={() => controlsVisiblity = false}
      on:focusout={() => controlsVisiblity = false}
